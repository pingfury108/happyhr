...
func OpenTestConnection() (db *gorm.DB, err error) {
	...
	case "mysql":
		...
		if dbDSN == "" {
			dbDSN = "gorm:gorm@tcp(localhost:3306)/gorm?charset=utf8&parseTime=True&loc=Local"
		}
		db, err = gorm.Open(mysql.Open(dbDSN), &gorm.Config{})
	case "postgres":
		log.Println("testing postgres...")
		if dbDSN == "" {
			dbDSN = "user=gorm password=gorm host=localhost dbname=gorm port=5432 sslmode=disable TimeZone=Asia/Shanghai"
		}
